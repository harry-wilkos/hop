...
from hop.my.util import set_fps
...
def handle_change():
    ...
    if cmds.attributeQuery("loadedShot", node="defaultRenderGlobals", exists=True):
        ...
        if id:
            ...
            if not shot:
                print("deleted")
            elif end_frame <= shot["start_frame"] or start_frame >= shot["end_frame"]:
                ...
                if result == 0:
                    ...
                    scene_objects = cmds.ls(long=True)
                    plate = [obj for obj in scene_objects if "->|Plate" in obj][1]
                    cmds.setAttr(
                        f"{plate}.frameOffset", shot["start_frame"] - start_frame
                    )

            # elif start_frame != shot["start_frame"]:
            #     print("shift_keyframes")
...
def create_shot(shot: dict | None = None):
    ...
    if not shot:
        shot = load_shot()
        if cmds.attributeQuery("scriptJob", node="defaultRenderGlobals", exists=True):
    ...
    if shot and shot["cam"] and shot["cam_path"]:
        ...
        cmds.playbackOptions(
            minTime=shot["start_frame"] - shot["padding"],
            maxTime=shot["end_frame"] + shot["padding"],
        )
        cmds.currentTime(shot["start_frame"] - shot["padding"])
        ...
        cmds.rename("|".join(cam_path[:-2]), "Shot")
        ...
        cmds.setAttr("defaultRenderGlobals.loadedShot", str(shot["_id"]), type="string")
        ...
        if not cmds.attributeQuery(
            ...
            cmds.addAttr("defaultRenderGlobals", longName="shotPath", dataType="string")
        cmds.setAttr(
            "defaultRenderGlobals.shotPath",
            "Shot",
            type="string",
        ...
        if not cmds.attributeQuery("frame", node="defaultRenderGlobals", exists=True):
            cmds.addAttr("defaultRenderGlobals", longName="frame", dataType="float2")
        cmds.setAttr(
            "defaultRenderGlobals.frame",
            shot["start_frame"] - shot["padding"],
            shot["end_frame"] + shot["padding"],
            type="float2",
        )
